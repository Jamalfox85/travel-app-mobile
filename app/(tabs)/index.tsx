...
export default function HomeScreen() {
  ...
  return (
    <ParallaxScrollView headerBackgroundColor={{ light: "#A1CEDC", dark: "#1D3D47" }} headerImage={<Image source={require("@/assets/images/index-bg.jpg")} style={styles.bgImg} />}>
      ...
      <View style={styles.contentContainer}>
        ...
        {tripData && tripData.filter((trip) => moment(trip.End_date) >= moment()).map((trip, index) => <TripRecord key={index} trip={trip} />)}
      </View>
      <View style={styles.contentContainer}>
      ...
      <CreateTripDrawer isOpen={createTripModalVisible} updateVisibility={(state) => setCreateTripModalVisible(state)} />
    </ParallaxScrollView>
  );
}
